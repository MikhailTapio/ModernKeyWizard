...
public class KeyWizardScreen extends GameOptionsScreen {
    ...
    protected void init() {
        ...
        this.screenToggleButton = new TexturedButtonWidget(this.width - 22, this.height - 22, 20, 20, 20, 0, 20, ModernKeyWizard.SCREEN_TOGGLE_WIDGETS, 40, 40, (btn) -> this.client.openScreen(new ControlsOptionsScreen(this.parent, this.gameOptions)));
        ...
        this.resetAll = new ButtonWidget(bindingListWidth + 117, this.height - 23, 70, 20, new TranslatableText("controls.resetAll"), (btn) -> {
            ...
            client.openScreen(new ResetAllConfirmScreen(b -> {
                if (b) {
                    for (KeyBinding k : this.gameOptions.keysAll) ((IKeyBinding) k).setToDefault();
                    KeyBinding.updateKeysByCode();
                }
